...
export function getUserStatus(
  ...
  if (user.isSupport) {
    // SupportStatus key is not in new lang types; falls back to key string
    return (lang as any)('SupportStatus') as string;
  }

  if (user.type && user.type === 'userTypeBot') {
    if (user.botActiveUsers) {
      // BotUsers key is not in new lang types; falls back to key string
      return (lang as any)('BotUsers', user.botActiveUsers, 'i') as string;
    }
    return lang('Bot');
  }
