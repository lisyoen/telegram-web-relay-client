  ...
  switch (update['@type']) {
    ...
    case 'newMessage': {
      ...
      if (!isLocal || chat.id === SERVICE_NOTIFICATIONS_USER_ID) {
        // v0.21: client-side +1 자체 가산 제거. TDLib server 가 newMessage 직후
        // chatReadInbox(권위 unread_count) 를 follow-up emit 하므로 그 값으로 set 한다.
        // 본 fork 는 unreadCount 를 서버 권위로 운용한다 (telegram-tt 원본의 gramjs
        // self-managed 모델과 다름).
        ...
        // forum/topic 의 unreadCount 는 TDLib 가 별도 chat 단위로 권위값을 주지 않으므로
        // 기존 client-side +1 가산 유지(주제별 안읽음은 TDLib 가 별도 동기화 채널 없음).
    ...
    case 'updateChatIsMuted': {
      const { chatId, isMuted } = update;
      const chat = global.chats.byId[chatId];
      if (!chat) {
        return undefined;
      }
      return updateChat(global, chatId, { isMuted });
    }
