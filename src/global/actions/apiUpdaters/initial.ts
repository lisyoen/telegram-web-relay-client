...
import { clearTg2SessionFlag, setTg2SessionFlag, updateSessionUserId } from '../../../util/sessions';
import { forceUpdateCache, getIsCaching, setupCaching } from '../../cache';
...
function onUpdateAuthorizationState<T extends GlobalState>(global: T, update: ApiUpdateAuthorizationState) {
  ...
  switch (authState) {
    case 'authorizationStateLoggingOut':
      ...
      // Drop the v2 auth marker promptly so a refresh during logout does not
      // restore a stale cached global tied to the prior account.
      clearTg2SessionFlag();
    ...
    case 'authorizationStateReady': {
      ...
      // v2 uses server-side TDLib; no GramJS saveSession is dispatched here, so
      // cache.ts never flips isCaching=true and client state (settings.language,
      // theme, etc.) is dropped on every refresh. Set the v2 marker and start
      // caching directly. forceUpdateCache writes immediately so the very first
      // refresh after auth-ready has a cache to restore.
      setTg2SessionFlag();
      if (!getIsCaching()) {
        setupCaching();
        forceUpdateCache();
...
function onUpdateConnectionState<T extends GlobalState>(
  ...
  if (global.isSynced) {
    ...
    // 037: 재접속 시 열린 chat에 권위 fetch 기반 reconcile — loadViewportMessages 캐시/앵커 게이트 우회
    if (connectionState === 'connectionStateReady') {
      const seen = new Set<string>();
      Object.entries(global.byTabId).forEach(([tabIdStr]) => {
        const msgList = selectCurrentMessageList(global, Number(tabIdStr));
        if (!msgList) return;
        const key = `${msgList.chatId}:${String(msgList.threadId)}`;
        if (!seen.has(key)) {
          seen.add(key);
          actions.reconcileChatMessages({ chatId: msgList.chatId, threadId: msgList.threadId });
        }
      });
    }
  }
