...
addActionHandler('loadCommonChats', async (global, actions, payload): Promise<void> => {
  ...
  // v0.48: TDLib(server.js fetchCommonChats) 경로는 result.chats 도 함께 반환한다.
  // GroupChatInfo 가 global.chats[chatId] 를 lookup 하므로, 공통 그룹 객체가 글로벌
  // 채팅 캐시에 없으면 빈 줄로 보인다. gramjs 경로는 invokeRequest 의 _processUpdate
  // 가 자동 머지하지만 TDLib 경로는 명시 머지 필요.
  const chats = (result as any).chats as any[] | undefined;
