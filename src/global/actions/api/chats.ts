...
import { debounce, onTickEnd, pause, throttle } from '../../../util/schedulers';
...
addActionHandler('openChat', (global, actions, payload): ActionReturnType => {
  ...
  // v0.21: 채팅 open 시 항상 서버 markRead 발화 (client unreadCount 가드 우회).
  // observer 기반 markMessageListRead 는 memoFirstUnreadIdRef 미설정 / 가드 통과
  // 실패 시 callApi 미호출 → TDLib viewMessages 미발화 → PC 안읽음 전파 실패.
  // chat.lastMessage.id 로 마지막 메시지까지 권위적으로 마킹한다 (idempotent).
  const lastMessageId = selectChatLastMessageId(global, id);
  if (lastMessageId && chat && !chat.isForum) {
    void callApi('markMessageListRead', { chat, threadId: MAIN_THREAD_ID, maxId: lastMessageId });
  }
  ...
  // [041] 그룹 open 시 멤버 선로드 — chatFullInfo.members 빈 경우 1회 트리거, @ 멘션 드롭다운 확보
  if (chat && (isChatBasicGroup(chat) || isChatSuperGroup(chat))) {
    const existingMembers = selectChatFullInfo(global, id)?.members;
    if (!existingMembers?.length) {
      onTickEnd(() => actions.loadMoreMembers({ chatId: id }));
    }
  }
});
...
addActionHandler('updateChatMutedState', (global, actions, payload): ActionReturnType => {
  ...
  // 낙관적 로컬 갱신: relay 왕복/리프래시 없이 메뉴가 즉시 음소거 상태를 반영하도록
  // chat.isMuted 를 즉시 업데이트한다 (009 의 getIsChatMuted 폴백이 이 값을 읽음).
  // mutedUntil 의미: 0/UNMUTE_TIMESTAMP=해제, 미래 ts=음소거, 2147483647(MAX_INT_32)=영구 음소거.
  const isMuted = mutedUntil ? getServerTime() < mutedUntil : false;
...
addActionHandler('loadMoreMembers', async (global, actions, payload): Promise<void> => {
  ...
  // Add users to global state so member names can be displayed
  if (users && users.length > 0) {
    const usersById = users.reduce<Record<string, ApiUser>>((acc, user) => {
      acc[user.id] = user;
      return acc;
    }, {});
    global = addUsers(global, usersById);
  }
...
export async function loadFullChat<T extends GlobalState>(
  ...
  // v0.55: 그룹/슈퍼그룹 분기는 fetchFullChat 가 {botCommands} 만 채워서 반환.
  // replaceChatFullInfo 로 전체 교체 시 기존 members/adminMembersById(다른 액션이
  // 채워둔 chatFullInfo 필드) 가 통째로 날아가 @멘션·멤버 회귀 발생.
  // 그룹은 updateChatFullInfo(머지) 로 botCommands 만 추가, 1:1/채널은 종전대로
  // replaceChatFullInfo(전체 교체) 유지.
  if (chat.type === 'chatTypeBasicGroup' || chat.type === 'chatTypeSupergroup') {
    global = updateChatFullInfo(global, chat.id, fullInfo);
  } else {
