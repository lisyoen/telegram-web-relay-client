...
export async function callApiOptional<T = any>(method: string, params?: any): Promise<T | undefined> {
  try {
...
export function fetchGlobalChatSearchResults(query: string) {
  return Promise.all([
    callApiOptional('searchChats', { query }),
    callApiOptional('fetchSponsoredPeer', { query }),
  ]);
}
...
addActionHandler('setGlobalSearchQuery', (global, actions, payload): ActionReturnType => {
  ...
  if (query && !chatId) {
    void searchThrottled(async () => {
      const [searchResult, sponsoredResult] = await fetchGlobalChatSearchResults(query);
      ...
      if (searchResult.chats?.length) {
        global = updateChats(global, buildCollectionByKey(searchResult.chats, 'id'));
      }
      if (searchResult.users?.length) {
        global = addUsers(global, buildCollectionByKey(searchResult.users, 'id'));
      }
      if (searchResult.userStatusesById) {
        global = addUserStatuses(global, searchResult.userStatusesById);
      }
...
addActionHandler('searchPopularBotApps', async (global, actions, payload): Promise<void> => {
  ...
  const result = await callApiOptional('fetchPopularAppBots', { offset });
...
addActionHandler('checkSearchPostsFlood', async (global, actions, payload): Promise<void> => {
  ...
  const result = await callApiOptional('checkSearchPostsFlood', query);
...
async function searchMessagesGlobal<T extends GlobalState>(global: T, params: {
  ...
  if (peer) {
    ...
    result = await callApiOptional('searchMessagesGlobal', {
      query,
      offsetRate,
      offsetId,
      offsetPeer,
