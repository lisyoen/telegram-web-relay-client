...
import { parseThreadHash } from '../../../util/routing';
...
import { computeReconcileDeletions } from '../../../api/tdlib/reconcile';
import {
  addChatMessagesById,
  addUnreadMentions,
  clearMessageSummary,
...
addActionHandler('reconcileChatMessages', async (global, actions, payload): Promise<void> => {
  ...
  const cachedMsgs = selectChatMessages(global, chatId);
  if (!cachedMsgs) return;

  const serverIds = messages.map((m) => m.id);
  const cachedIds = Object.keys(cachedMsgs).map(Number);
  const pendingIds = cachedIds.filter((id) => Boolean(cachedMsgs[id]?.sendingState));
  const lastMessageId = selectChat(global, chatId)?.lastMessage?.id
    ?? (serverIds.length > 0 ? Math.max(...serverIds) : 0);

  if (serverIds.length > 0 && lastMessageId > 0) {
    const { toDelete, tailRemoved, midRemoved } = computeReconcileDeletions({
      cachedIds, serverIds, lastMessageId, pendingIds,
    });
    if (toDelete.length > 0) {
      // eslint-disable-next-line no-console
      console.log(`[reconcile] chat:${chatId} removed:${toDelete.length} (tail:${tailRemoved} mid:${midRemoved})`);
      global = deleteChatMessages(global, chatId, toDelete);
      setGlobal(global);
    }
  }
});
...
addActionHandler('openUrl', (global, actions, payload): ActionReturnType => {
  ...
  // tg2 등 자기 origin 링크: 새 탭 대신 현재 탭에서 앱내 채팅 이동
  if (parsedUrl.origin === window.location.origin && parsedUrl.hash) {
    const h = parsedUrl.hash.replace(/^#/, '');
    const parsed = parseThreadHash(h);
    if (parsed?.chatId) {
      window.location.hash = h;
      actions.openThread({
        chatId: parsed.chatId, threadId: parsed.threadId, type: parsed.type, tabId,
      });
      return;
    }
  }
