...
export function calculateMediaDimensions({
  ...
  // v0.45: mediaType 누락 방어 — buildApiPhoto 미경유(optimistic send·state hydration·legacy cache·외부 update)로
  // mediaType 가 없지만 sizes 를 가진 raw photo 를 photo 로 취급. 없으면 extendedPreview 분기 → DEFAULT 100x100 로 작게 렌더됨.
  const isPhoto = media.mediaType === 'photo' || (!('mediaType' in media) && 'sizes' in media);
  const isVideo = media.mediaType === 'video';
  const isWebPagePhoto = isPhoto && isInWebPage;
  const isWebPageVideo = isVideo && isInWebPage;
  const { width, height } = isPhoto
    ? calculateInlineImageDimensions(media as ApiPhoto, isOwn, asForwarded, isWebPagePhoto, noAvatars, isMobile)
