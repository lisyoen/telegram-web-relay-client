...
import { computeShouldRenderGreeting } from './helpers/shouldRenderGreeting';
...
const MessageList = ({
  ...
  const shouldRenderGreeting = computeShouldRenderGreeting({
    chatId,
    isChatWithSelf: Boolean(isChatWithSelf),
    isBot: Boolean(isBot),
    isAnonymousForwards: Boolean(isAnonymousForwards),
    type,
    hasMessageGroups: Boolean(messageGroups),
    hasLastMessage: Boolean(lastMessage),
    hasMessageIds: Boolean(messageIds),
    // Used to avoid flickering when deleting a greeting that has just been sent
    listItemCount: listItemElementsRef.current?.length ?? 0,
  });
