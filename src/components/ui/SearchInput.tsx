...
const SearchInput: FC<OwnProps> = ({
  ...
  function handleChange(event: ChangeEvent<HTMLInputElement>) {
    ...
    if (isComposingRef.current) {
      setCompositionValue(currentTarget.value);
  ...
  const handleKeyDown = useLastCallback((e: KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Enter') {
      if (onEnter) {
        e.preventDefault();
        onEnter();
  ...
  return (
    ...
    >
      ...
      <input
        ...
        value={isComposingRef.current ? compositionValue : value}
        disabled={disabled}
