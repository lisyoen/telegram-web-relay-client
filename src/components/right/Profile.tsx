...
const Profile = ({
  ...
  const tabs = useMemo(() => {
    ...
    // Web K 정합: 1:1 유저 프로필에서는 gif/voice 탭을 숨긴다(빈 탭).
    // 채널/봇/own/saved/그룹멤버 경로는 종전과 동일하게 모두 노출.
    const isOneToOneUser = isUser && !isChannel && !isBot;
    if (!isOwnProfile) {
      const baseTabs = isOneToOneUser ? TABS.filter((t) => t.type !== 'gif') : TABS;
      arr.push(...baseTabs);
    }

    // Voice messages filter currently does not work in forum topics. Return it when it's fixed on the server side.
    if (!isTopicInfo && !isOwnProfile && !isOneToOneUser) {
    ...
    hasCommonChatsTab, isUser, isChannel, isBot, similarChannels?.length, similarBots?.length, lang, isOwnProfile,
