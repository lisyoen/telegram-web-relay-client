...
const MediaViewer = ({
  ...
  const isContextMenuOpen = Boolean(contextMenuAnchor);

  const handleMediaContextMenu = useLastCallback((e: React.MouseEvent<HTMLDivElement>) => {
    if (isMobile) return;
    e.preventDefault();
    e.stopPropagation();
  ...
  const messageMediaIds = useMemo(() => {
    const ids = withDynamicLoading
      ? collectedMessageIds
      : getMessageContentIds(chatMessages || {}, collectedMessageIds || [], contentType || 'media');
    // runtime order is descending(newest-first); reverse to ascending(old->new) so left=prev/older, right=next/newer
    return ids ? [...ids].reverse() : ids;
  ...
  (global): Complete<StateProps> => {
    ...
    if (chatId && threadId && messageId) {
      if (withDynamicLoading && (isOriginInline || isOriginAlbum)) {
        const currentSearch = selectChatMediaSearch(global, chatId, threadId);
