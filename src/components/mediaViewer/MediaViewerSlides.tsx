...
const MediaViewerSlides: FC<OwnProps> = ({
  ...
  const changeSlideRef = useRef<((direction: number) => boolean) | undefined>();
  ...
  // isLoadingMoreMedia를 deps에 포함: 검색 완료(false로 전환) 시 getNextItem 내부
  // messageMediaIds가 채워지지만 getNextItem은 useLastCallback(안정 참조)이라 deps만으로는
  // 재계산되지 않아 화살표가 영구히 숨겨지는 회귀([053])를 방지.
  const [prevItem, nextItem] = useMemo(() => {
    if (activeItem === undefined) return [undefined, undefined];
    return [getNextItem(activeItem, -1), getNextItem(activeItem, 1)];
  }, [activeItem, getNextItem, isLoadingMoreMedia]);

  const handlePrevClick = useLastCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    changeSlideRef.current?.(-1);
  });

  const handleNextClick = useLastCallback((e: React.MouseEvent) => {
    e.stopPropagation();
    changeSlideRef.current?.(1);
  });
    ...
    <div className="MediaViewerSlides" ref={containerRef}>
      ...
      {hasPrev && !isScaled && !IS_TOUCH_ENV && (
        <Button
          ...
          className={buildClassName('media-viewer-nav-arrow', 'media-viewer-nav-arrow--prev', lang.isRtl && 'rtl')}
      ...
      {hasNext && !isScaled && !IS_TOUCH_ENV && (
        <Button
          ...
          size="smaller"
          color="translucent-white"
          className={buildClassName('media-viewer-nav-arrow', 'media-viewer-nav-arrow--next', lang.isRtl && 'rtl')}
          ariaLabel={lang('Next')}
          iconName="next"
