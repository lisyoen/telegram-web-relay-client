...
function checkIfShouldNotify(chat: ApiChat, message: Partial<ApiMessage>) {
  const global = getGlobal();
  const isChatMuted = Boolean(chat.isMuted)
    || getIsChatMuted(chat, selectNotifyDefaults(global), selectNotifyException(global, chat.id));
...
export async function notifyAboutMessage({
  ...
  if (checkIfPushSupported() && navigator.serviceWorker?.controller) {
