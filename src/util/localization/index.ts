...
import KO_LANG_FALLBACK from './koLangFallback';
...
function getString(langKey: LangKey, count: number) {
  // v0.55 [038]: langCode 'ko' → 큐레이션된 정적 한국어 폴백(KO_LANG_FALLBACK)을 원격
  // weba/ko 팩보다 우선 적용. 원격 팩이 OnlineCount/NMembers/Subscribers 등을 Android
  // '%1$d' 형식으로 내려 신규 useLang 치환부({count})와 불일치 → 플레이스홀더가 그대로
  // 노출되는 버그 교정. KO_LANG_FALLBACK 에 없는 키는 종전대로 원격 → 영문 fallback 순.
  let langPackStringValue = language?.langCode === 'ko' ? KO_LANG_FALLBACK[langKey] : undefined;

  langPackStringValue ||= !FORCE_FALLBACK_LANG ? langPack?.strings[langKey] : undefined;
