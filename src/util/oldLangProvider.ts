...
import OLD_LANG_FALLBACK from './oldLangFallback';
...
// remote langPack 미적재 시에도 컨텍스트 메뉴 등 핵심 oldLang 키가 raw 로 노출되지 않도록
// 정적 영문 fallback 으로 초기화. remote 적재 시 fallback 위에 merge.
let langPack: ApiOldLangPack | undefined = { ...OLD_LANG_FALLBACK };
...
export async function oldSetLanguage(langCode: LangCode, callback?: NoneToVoidFunction) {
  ...
  console.log('[LANGDIAG] oldSetLanguage enter', langCode);
  ...
  // eslint-disable-next-line no-console
  console.log('[LANGDIAG] cache fetch', langCode, 'hit=', !!newLangPack, 'keys=', newLangPack ? Object.keys(newLangPack).length : 0);
  if (!newLangPack) {
  ...
  // fallback 위에 remote pack 을 덮어 누락 키는 fallback 값을 유지.
  langPack = { ...OLD_LANG_FALLBACK, ...newLangPack };
  // eslint-disable-next-line no-console
  console.log('[LANGDIAG] merged langPack keys=', Object.keys(langPack || {}).length, 'sampleSettings=', (langPack as any)?.['Settings']);
...
async function fetchRemote(langCode: string): Promise<ApiOldLangPack | undefined> {
  ...
  // eslint-disable-next-line no-console
  console.log('[LANGDIAG] remote oldFetchLangPack', langCode, 'ok=', !!(remote && remote.langPack), 'keys=', (remote && remote.langPack) ? Object.keys(remote.langPack).length : 0);
  if (remote) {
