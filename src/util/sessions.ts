...
export function hasTg2SessionFlag() {
  return localStorage.getItem(TG2_SESSION_FLAG) === '1';
}

export function setTg2SessionFlag() {
  localStorage.setItem(TG2_SESSION_FLAG, '1');
}

export function clearTg2SessionFlag() {
  localStorage.removeItem(TG2_SESSION_FLAG);
}
...
export function hasStoredSession() {
  ...
  // v2 server-side TDLib path: no GramJS session is written; the v2 auth handler
  // sets this flag once authorizationStateReady is reached. Without this branch,
  // cache.ts/loadCache() drops cached client state on every refresh.
  if (hasTg2SessionFlag()) return true;
