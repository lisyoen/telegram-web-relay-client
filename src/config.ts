...
// v2 uses server-side TDLib for the auth session; no GramJS session is stored
// in localStorage. This flag is written when v2 reaches authorizationStateReady
// so that client-side caching paths (which gate on hasStoredSession) can detect
// an authenticated v2 session and persist client state like settings.language.
export const TG2_SESSION_FLAG = 'tg2-has-session';
